      ...
      <div className="flex flex-1 mt-16 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto blueprint-grid">
          <div className="p-panel-padding max-w-[1600px] mx-auto space-y-6">
            ...
            {/* Hero Section — Defect Visualization */}
            <section className="relative w-full h-[520px] bg-surface-container-lowest border border-outline-variant overflow-hidden">
              ...
              {/* SEM-like visualization using CSS */}
              <div className="absolute inset-0 bg-surface-container-lowest">
                <div className="w-full h-full relative overflow-hidden">
                  {/* Simulated semiconductor circuit pattern */}
              ...
              <div className="absolute inset-0 pointer-events-none">
                ...
                {/* Scan line animation */}
                <div className="absolute left-0 right-0 h-px bg-primary-fixed/30"
                  style={{ animation: 'scanline 4s linear infinite', top: '0%' }} />
              </div>

              {/* AI Terminal Overlay */}
              <div className="absolute bottom-6 left-6 w-80 bg-surface-container/90 backdrop-blur-xl border border-secondary p-4 shadow-2xl z-10">
                <div className="flex justify-between items-center mb-4 border-b border-secondary/30 pb-2">
                  ...
                  <span className="material-symbols-outlined text-secondary text-sm select-none">terminal</span>
                </div>
                <div className="font-data-sm text-data-sm text-on-surface space-y-1">
                  {AI_LINES.slice(0, visibleLines).map(({ prefix, text, type }, i) => (
                    <p key={i} className={type === 'ai' ? 'pl-4 border-l border-secondary/30 text-secondary' : 'opacity-70'}>
                      {type !== 'ai' && <span className="mr-1">{prefix}</span>}
                      {type === 'ai' && <span className="mr-1">+</span>}
                      {text}
                    </p>
                  ))}
                ...
                {visibleLines >= AI_LINES.length && (
                  <button
                    ...
                    className="mt-4 w-full py-2 bg-primary-container text-on-primary-container font-data-sm text-data-sm uppercase hover:opacity-80 transition-opacity"
                  >
                    View Fix Protocol →
                  </button>
                )}
              ...
              <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
                <div className="bg-error-container border border-error px-3 py-1">
                  <span className="font-label-caps text-label-caps text-on-error-container">STG-04 COW_BONDING :: CRITICAL</span>
                </div>
              </div>
            </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* FMECA Card */}
              <div className="bg-surface-container border border-outline-variant p-6 relative col-span-1 lg:col-span-2">
                <div className="absolute top-4 right-4">
                  <span className="font-label-caps text-label-caps text-on-surface-variant">MOD-FMECA-01</span>
                </div>
                <h3 className="font-headline-lg text-headline-lg text-primary mb-6">FMECA Scoring</h3>

                <div className="grid grid-cols-3 gap-4 mb-8">
                  {[
                    { label: 'Severity (S)',    value: '8', color: 'text-error' },
                    { label: 'Occurrence (O)', value: '4', color: 'text-primary-fixed' },
                    { label: 'Detection (D)',  value: '3', color: 'text-secondary' },
                  ].map(({ label, value, color }) => (
                    <div key={label} className="border border-outline/30 p-4">
                      <span className="font-label-caps text-label-caps text-on-surface-variant block mb-1">{label}</span>
                      <span className={`font-data-display text-2xl ${color}`}>{value}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex items-end justify-between border-t border-outline-variant pt-4">
                  <div>
                    ...
                    <span className="font-headline-xl text-headline-xl text-primary">96</span>
                  </div>
                  <div className="flex flex-col gap-2 items-end">
                    <div className="bg-error-container text-on-error px-3 py-1 border border-error">
                      <span className="font-data-sm text-data-sm uppercase">Critical Threshold Exceeded</span>
                    </div>
                    <button
                      onClick={() => navigate('/fix')}
                      className="bg-primary-container text-on-primary-container px-4 py-2 font-data-sm text-data-sm uppercase hover:opacity-80 transition-opacity"
                    >
              ...
              {/* Live Scrap Cost */}
              <div className="bg-surface-container border border-outline-variant p-6 relative flex flex-col justify-between">
                <div className="absolute top-4 right-4">
                  <span className="font-label-caps text-label-caps text-on-surface-variant">MOD-COST-02</span>
                </div>
                <div>
                  <h3 className="font-data-display text-data-display text-on-surface-variant mb-2 uppercase">
                    Live Scrap Cost
                  </h3>
                  <div className="font-headline-xl text-headline-xl text-primary-fixed font-data-display tracking-tighter tabular-nums">
                    ${dollars}
                ...
                <div className="mt-8 border-t border-outline-variant pt-4">
                  ...
                  <p className="font-data-sm text-data-sm text-error mt-2">&gt; Target pad misalignment &gt; 25%</p>
                  <p className="font-data-sm text-data-sm text-error">&gt; Solder bridge detected on pins 4-5</p>
                </div>
              </div>
            </div>
          </div>
